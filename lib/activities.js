...
Meteor.methods({
  createActivity: function(activity, loc) {
    ...
    activity.userAvatar = "/img/app/max.jpg";
    activity.userName = "Max";
    activity.date = new Date;
    
    if (! this.isSimulation && loc)
      activity.place = "That Lovely Place, Tehran";
...
Meteor.startup(function() {
  if (Meteor.isServer && Activities.find().count() === 0) {
    Activities.insert({
      recipeName: 'her',
      text: 'Thanks for being in my life...last night was incredible',
      image: '/img/activity/activity-placeholder-strawberry-640x640.jpg',
      userAvatar: '/img/app/max.jpg',
      userName: 'Max Mirkia',
      place: 'Evin Hotel Residence, Tehran',
      date: new Date
    });
  }
});
